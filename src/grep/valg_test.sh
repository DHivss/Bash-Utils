...
SUCCESS=0
ERROR=0
LEAK_RES="ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)"
test()
{
    for ((i=0; i<${#FILES[@]}; i++))
    do
    grep $var ${FILES[$i]} > check.txt
    valgrind --log-file=leaks.txt ./s21_grep $var1 ${FILES[$i]} > check2.txt
    (( COUNTER++ ))
    if [ "$LEAK_RES" == "$(grep -o "ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)" leaks.txt)"  ]
    then
    (( SUCCESS++ ))
    ...
    rm check.txt check2.txt leaks.txt
}
...
test2()
{
    ...
    valgrind --log-file=leaks.txt ./s21_grep $var4 ${FILES[$i]} $var1 > check2.txt
    (( COUNTER++ ))
    if [ "$LEAK_RES" == "$(grep -o "ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)" leaks.txt)"  ]
    then
    (( SUCCESS++ ))
